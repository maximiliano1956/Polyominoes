...
int Decode(void);
int Dec_par(FILE *,char *,char **,int *,int *,char ***);
...
int Ugu(char **,char **);
int compare(char,char,char **,char **);
...
void DrawField(Drawable,int,int,char **);
...
int Dec_par(FILE *pf,char *tip_par,char **name_par,int *nr,int *nc,char ***dati)
{
char riga[256];
unsigned char i;
unsigned char j;
...
while (fgets(riga,sizeof(riga),pf))
	...
	*tip_par=ch2;
	...
	if (*nr && *nc)
		{
		if ((*dati=malloc((*nr)*sizeof(char **)))==NULL)
			{
			printf("\nNot enough memory!\n\n");
			return(ERR);
			}
...
if ((mat=malloc(max_dim*sizeof(char **)))==NULL)
	{
	printf("\nNot enough memory!\n\n");
	exit(1);
	}
...
for (trasf=0;trasf<7;trasf++)
	...
	if (ugu==0)
		...
		if ((mat1=malloc(pun_pez->nr*sizeof(char **)))==NULL)
			{
			printf("\nNot enough memory!\n\n");
			exit(1);
			}
		...
		for (nr=0;nr<pun_pez->nr;nr++)
			if ((mat1[nr]=malloc(pun_pez->nc))==NULL)
				{
				printf("\nNot enough memory!\n\n");
				exit(1);
				}
		...
		*frst=pset->testa_def;
		...
		while (pfield)
			...
			if (!strlen(f_nm) || !strcmp(pfield->Field_name,f_nm))	/* trovato field */
				...
				if (((*fld)->field=
					malloc(((*fld)->nr)*sizeof(char **)))==NULL)
					{
					printf("\nNot enough memory!\n\n");
					return(ERR);
					}
...
for (nc=0;nc<p_field->nc;nc++)
	for (nr=0;nr<p_field->nr;nr++)
		if (p_field->field[nr][nc]<0) p_field->field[nr][nc]=0;
return(esito);
}
...
if (((*sol2)->field=malloc((solutions.nr)*sizeof(char **)))==NULL)
	{
	printf("\nNot enough memory!\n\n");
	exit(1);
	}
...
for (nr=0;nr<solutions.nr;nr++)
	if (((*sol2)->field[nr]=malloc(solutions.nc))==NULL)
		{
		printf("\nNot enough memory!\n\n");
		exit(1);
		}
...
int Ugu(char **field1,char **field2)
{
int nr;
int nc;
int n=0;
...
if ((mat=malloc((solutions.nr)*sizeof(char **)))==NULL)
	{
	printf("\nNot enough memory!\n\n");
	exit(1);
	}
for (nr=0;nr<solutions.nr;nr++)
	if ((mat[nr]=malloc(solutions.nc))==NULL)
		{
		printf("\nNot enough memory!\n\n");
		exit(1);
		}
...
int compare(char Nr,char Nc,char **mat1,char **mat2)
{
int nr;
int nc;
...
void DrawField(Drawable d,int Nr,int Nc,char **field)
{
int nr;
int nc;
int x;
...
void WSol(FILE *fp,int numsol,char **field,char *ptime)
{
int nr;
int nc;
int count;
